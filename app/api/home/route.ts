...
export async function GET() {
  ...
  const [events, tours, news, partners, reviews, artistProfiles, artistPhotos, profiles] = await Promise.all([
    ...
    supabase.from("artist_profiles").select("id, user_id, show_in_artist_list").limit(30),
  ...
  const artists = (artistProfiles.data ?? []).map((ap) => {
    const showPhoto = (ap as unknown as { show_in_artist_list: boolean }).show_in_artist_list !== false
    return {
      id: ap.id,
